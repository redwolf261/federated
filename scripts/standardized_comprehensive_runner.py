...
# Add project root to Python path BEFORE importing flex_persona modules
...
def run_comprehensive_experiments():
    ...
    # GPU-optimized regimes (4x larger batch sizes for better GPU utilization)
    regimes = {
        "high_het": {
            "name": "High Heterogeneity (GPU Optimized: 256 samples, 3 epochs, batch=128)",
            "description": "High heterogeneity regime with GPU-optimized batch size",
            "rounds": 20,
            "local_epochs": 3,
            "batch_size": 128,  # 4x larger: 32 -> 128 for better GPU utilization
            "max_samples_per_client": 256,
            "learning_rate": 0.01,
        },
        "low_het": {
            "name": "Low Heterogeneity (GPU Optimized: 1000 samples, 1 epoch, batch=256)",
            "description": "Low heterogeneity regime with GPU-optimized batch size",
            "rounds": 30,
            "local_epochs": 1,
            "batch_size": 256,  # 4x larger: 64 -> 256 for better GPU utilization
    ...
    print("=" * 100)
    print("[GPU-OPTIMIZED] 10-SEED EXPERIMENT SUITE WITH STANDARDIZED ARTIFACTS")
    print("=" * 100)
    print("[SPEEDUP] GPU Optimization: 4x larger batch sizes (high_het: 128, low_het: 256)")
        ...
        for method in methods:
            ...
            for seed_idx, seed in enumerate(seeds, 1):
                ...
                try:
                    ...
                    cfg.model.num_classes = 62  # FEMNIST has 62 classes, not 100!
